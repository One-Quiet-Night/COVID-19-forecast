...
metrics = {
    ...
    'FbSurvey_SmoothedTestedPositive14D': [
        {
            'data_source': 'fb-survey',
    ...
    'FbSurvey_SmoothedWantedTest14D': [
        {
            'data_source': 'fb-survey',
...
def conform_covidcast_id(env, covidcast_df):
    ...
    covidcast_geo_value_map = pd.concat([
        ...
        msa_counties_df[['id', 'CBSACode']].rename(columns={'CBSACode': 'geo_value'})
    ])

    covidcast_df = pd.merge(
        covidcast_df,
        covidcast_geo_value_map,
        on='geo_value',
        how='left'
    ).dropna(subset=['id'])
    return covidcast_df
...
def extract_df(covidcast_df):
    ...
    covidcast_df = covidcast_df.groupby(['id', 'dates', 'issue']).tail()
...
def load_data(env):
    ...
    for name, configs in metrics.items():
        ...
        for config in configs:
            start = timer()
            logger.info(f"Loading {config['data_source']}_{config['signal']}_{config['geo_type']}")
